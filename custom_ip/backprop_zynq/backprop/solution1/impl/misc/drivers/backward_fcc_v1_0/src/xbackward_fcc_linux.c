...
typedef struct {
    ...
    XBackward_fcc_uio_map maps[ MAX_UIO_MAPS ];
} XBackward_fcc_uio_info;

/***************** Variable Definitions **************************************/
static XBackward_fcc_uio_info uio_info;
...
static int uio_info_read_name(XBackward_fcc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XBackward_fcc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XBackward_fcc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XBackward_fcc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XBackward_fcc_Initialize(XBackward_fcc *InstancePtr, const char* InstanceName) {
	XBackward_fcc_uio_info *InfoPtr = &uio_info;
